...
for ii = 1:length(SNR_dB)
    ...
    for j= 1:n:length(coded_ipHat)
     ...
     soft_decoded_ip=[];
     for j= 1:n:length(y_coded)-n+1
         received_sum=sum(y_coded(j:j+n-1));
         if received_sum>= 0
            soft_dec_ip= 1;
         else
            soft_dec_ip= 0;
         end
         soft_decoded_ip=[soft_decoded_ip soft_dec_ip];
     end
    % counting the errors
    nErr(ii) = size(find([ip~=ipHat]),2);
    decoded_nErr(ii) = size(find([ip~=decoded_ip]),2);
    soft_decoded_nErr(ii) = size(find([ip~=soft_decoded_ip]),2);
end
simBer = nErr/N; % simulated ber
decoded_simBer = decoded_nErr/N; % simulated ber
soft_decoded_simBer = soft_decoded_nErr/N; % simulated ber
...
semilogy(SNR_dB,soft_decoded_simBer,'gp-','LineWidth',1);
axis([-3 15 10^-5 0.5])
grid on
legend('Uncoded BPSK AWGN-Theory','Uncoded BPSK AWGN-Simulation', ['Repetition ...' ...
    'Code BPSK AWGN-Simulation Hard Decision'],'Repetition Code BPSK AWGN-Simulation Soft Decision');
xlabel('SNR (dB)');
